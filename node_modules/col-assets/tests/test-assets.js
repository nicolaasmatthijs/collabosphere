      ...
      it('can be created', function(callback) {
        TestsUtil.getAssetLibraryClient(null, null, null, function(client, course, user) {
          UsersTestsUtil.assertGetMe(client, course, null, function(me) {
            ...
            bookmarkletClient.assets.bookmarklet.createLink(course, me.id, me.bookmarklet_token, 'Google', 'http://www.google.com', {'source': 'http://www.cnn.com'}, function(err, asset) {
              assert.ok(!err);
              assert.ok(asset);

              client.assets.getAsset(course, asset.id, function(err, asset) {
                assert.ok(!err);
                assert.ok(asset);

                client.assets.getAssets(course, null, null, function(err, assets) {
                  assert.ok(!err);
